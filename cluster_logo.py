...
class ImageComparer:
    ...
    def __new__(cls):
        if cls._instance is None:
            cls._instance = super(ImageComparer, cls).__new__(cls)
            cls._instance._initialize()
        return cls._instance

    def _initialize(self):
        self.orb = cv2.ORB_create(nfeatures=500, scaleFactor=1.3, edgeThreshold=15)
        self.bf = cv2.BFMatcher(cv2.NORM_HAMMING)

    def compare(self, img1_bytes, img2_bytes):
        try:
            ...
            kpA, desA = self.orb.detectAndCompute(img1, None)
            kpB, desB = self.orb.detectAndCompute(img2, None)
...
def process_similarity(domain1, data1, domain2, data2, threshold):
    if not data1 or not data2:
        return None
    similarity = image_comparer.compare(data1, data2)
